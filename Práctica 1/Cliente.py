...
def CrearMenu():
        ...
        try:
            ...
            for item in contenido:
                ...
                if os.path.isdir(item_path):
                    tree.insert('', 'end', values=(item + "\\",)) # Agregando un "/" al final para denotar una carpeta
                else:
    ...
    def boton_presionado(lado, accion, tree):
        ...
        if seleccion:
            ...
            if accion == "Subir":
                ...
                respuesta = messagebox.askquestion("Confirmar", f"¿Deseas eliminar '{archivo_seleccionado}'?", icon='warning')
                if respuesta == 'yes':
                    comando = f"ELIMINAR_{carpeta.upper()} {archivo_seleccionado}"
            
            elif accion == "Renombrar":
                nuevo_nombre = simpledialog.askstring("Renombrar", f"Nuevo nombre para '{archivo_seleccionado}' (sin espacios):", initialvalue=archivo_seleccionado)
    ...
    for btn_text in botones_izquierdos:
        boton = ttk.Button(frame_izquierdo, text=btn_text, command=lambda accion=btn_text: boton_presionado("izquierdo", accion, tree_izquierdo))
    ...
    for btn_text in botones_derechos:
        boton = ttk.Button(frame_derecho, text=btn_text, command=lambda accion=btn_text: boton_presionado("derecho", accion, tree_derecho))
        boton.pack(fill=tk.X, pady=5)
